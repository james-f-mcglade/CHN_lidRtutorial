...
-   [LAStools Website](https://rapidlasso.de/product-overview/)
...
[Download LAZ files](/data/fm_laz.zip)
:::
...
LAStools commands can be called through R (or other programming languages) by constructing and calling the command as a string using functions shell(). For clarity, we will show the commands here as they would be run directly in a Windows command prompt.

If you would like to run these commands from R, you can use the following template:
...
RLAStools <- function(LAStoolsDir = "",
  ...
  if (!quiet) {
    glue::glue("Command took {round(difftime(t2, t1, units = 'secs'), 2)} seconds.")
  }
  return(cmd)
}

# Example to perform lastile
RLAStools(
  LAStoolsDir = "C:/path/to/LAStools/bin/",
  LAStool = "lastile",
  input = '"C:/lidar_project/raw/*.laz"',
  arguments = '-odir "C:/lidar_project/processed/tiled" -olaz -tile_size 500 -buffer 0',
  quiet = FALSE
)
